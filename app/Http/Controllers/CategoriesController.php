...
class CategoriesController extends Controller
{
    ...
    public function addCategory(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|string|between:10,255|unique:categories',
        ]);
    ...
    public function updateCategoryData($category_id, Request $request)
    {
        $validator = Validator::make(array_merge($request->all(), ["id" => $category_id]), [
            'id' => 'required|integer|exists:categories,id',
            'name' => 'string|between:10,255,unique:categories,name',
        ...
        if (array_key_exists('name', $newCategoryData)) {
            $newCategoryData['slug'] = Str::slug($newCategoryData['name'], "-");
        }
